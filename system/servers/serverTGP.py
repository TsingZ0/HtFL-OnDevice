...
class FedTGP(fl.server.strategy.FedAvg):
    def __init__(self,
        ...
        self.TGP_opt = torch.optim.SGD(
            self.TGP.parameters(), lr=self.args.learning_rate)
    ...
    def update_TGP(self, uploaded_protos):
        ...
        for _ in range(self.args.epochs):
            ...
            for proto, y in proto_loader:
                ...
                proto_gen = self.TGP(list(range(self.args.num_classes)))
                ...
                self.TGP_opt.zero_grad()
                loss.backward()
                self.TGP_opt.step()

        self.TGP.eval()
        global_protos = [self.TGP(i).detach() for i in range(self.args.num_classes)]
